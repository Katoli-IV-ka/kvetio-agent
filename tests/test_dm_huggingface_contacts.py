...
def test_get_hf_org_for_domain_finds_org():
    ...
    store.get_signals_for_company.return_value = [
        {"source_page_url": "https://huggingface.co/radai-robolab", "evidence_url": ""},
    ]
...
def test_get_hf_org_for_domain_skips_non_hf():
    store = MagicMock()
    store.get_signals_for_company.return_value = [
        {"source_page_url": "https://github.com/radai-robolab", "evidence_url": ""},
    ]
    result = get_hf_org_for_domain("radai.com", store)
...
def test_get_hf_org_skips_system_paths():
    store = MagicMock()
    store.get_signals_for_company.return_value = [
        {"source_page_url": "https://huggingface.co/models", "evidence_url": ""},
    ]
    result = get_hf_org_for_domain("radai.com", store)
    assert result is None
