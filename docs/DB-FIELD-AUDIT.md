...
## `bot_users` - allowlist Telegram bot
...
Если бот не используется, можно удалить таблицу вместе с `bot/access.py`. Если бот нужен - оставить.
...
## `bot_dialog_state` - состояние wizard `/run`
...
Удалять только если убрать multi-step wizard из бота.
