...
- Бот = триггер с параметрами (мастер `/run`/пресеты собирают конфиг → передают в `text` у `/fire`).
...
|---|---|---|
| `gateway.py` | **CHANGE** | Заменить `runs.enqueue(...)` в `/run` и `/quickrun` на POST `/fire` (новый клиент). `/status`,`/last` — читать `run_logs`, не `pipeline_runs`. Удалить `/cancel`, внутренний `/runs` API и `/runs/{id}/cancel` (обслуживали worker). Оставить: webhook, allowlist, `/digest`/`/hot`/`/stale`, `TelegramSender`, `/ping`/`/help`/`/whoami` |
| `access.py` | **KEEP** | allowlist (`bot_users`) нужен |
| `dialog.py` | **KEEP** | мастер `/run`; его выход теперь идёт в `text` у `/fire`, а не в очередь |
| `presets.py` | **KEEP** | пресеты параметров запуска |
...
| Файл | Вердикт | Что делать |
|---|---|---|
| `pipeline_task.md` | **CHANGE** | Сделать промптом рутины: убрать ожидание инъекции конфига от worker; научить читать опциональные параметры из `text` (segments/limit); убедиться, что финальный шаг — `notify.py` (Шаг 7 уже есть) |
...
| `bot_users` (`012_bot.sql`) | **KEEP** | allowlist |
| `bot_presets` (`012_bot.sql`) | **KEEP** | пресеты параметров |
| `bot_dialog_state` (`012_bot.sql`) | **KEEP** | состояние мастера `/run` |
...
| `test_bot_access.py` | **KEEP** | allowlist |
| `test_pipeline_prompts.py` | **CHANGE** | сверить с обновлённым `pipeline_task.md` (без инъекции конфига) |
...
1. **Рутина в облаке.** Создать routine на claude.ai/code: промпт = `pipeline_task.md`,
   репо = `kvetio-agent`, Environment с секретами + `pip install -r requirements.txt`,
   триггеры Schedule + API. Сохранить `ROUTINE_FIRE_URL` и `ROUTINE_TOKEN`.
2. **Промпт.** Обновить `pipeline_task.md` (см. таблицу), прогнать `Run now`, убедиться,
   что приходит Telegram-сводка от `notify.py`.
