...
Бот в новой схеме делает только одно: принимает твои команды в Telegram, проверяет
allowlist и дёргает `/fire` рутины. Он не запускает агента и не шлёт уведомления о
...
## Шаг 3 — Завести себя в allowlist (Supabase)

Бот игнорирует всех, кого нет в таблице `bot_users`. Роль `admin` даёт право
запускать рутину (`/run`, `/quickrun`).

В Supabase SQL Editor:

```sql
insert into bot_users (chat_id, username, role, active)
values ('<ТВОЙ_CHAT_ID>', '<твой_username>', 'admin', true);
```

(Таблица создаётся миграцией `sql/012_bot.sql` — она должна быть применена.)
...
| `SUPABASE_URL`, `SUPABASE_KEY` | проект Supabase | allowlist + чтение для `/status`,`/digest` |
...
1. `/ping` → `🏓 pong` (бот жив, без проверки allowlist).
2. `/whoami` → показывает твой `chat_id` и роль `admin` (значит allowlist работает).
3. `/quickrun` → бот POST-ит на `/fire`, в ответ присылает `session_id`/ссылку на сеанс.
4. Открыть сеанс на claude.ai/code — рутина выполняется; по завершении в Telegram
   приходит сводка от `scripts/notify.py` (это шлёт рутина, не бот).
5. `/status`, `/last`, `/digest`, `/hot`, `/stale` → отвечают данными из Supabase.

---

## Если что-то не работает

- **Бот молчит на все команды, кроме `/ping`** → тебя нет в `bot_users` или `active=false`
  (Шаг 3), либо неверный `chat_id`.
